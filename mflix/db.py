...
def get_db():
    ...
    if db is None:
        ...
        db = g._database = MongoClient(MFLIX_DB_URI, maxPoolSize = 50,
                                       w="majority", wTimeoutMS=2500,)[MFLIX_DB_NAME]
    return db
...
def get_movies_by_country(countries):
    ...
    try:
        ...
        cursor = movies.find( {"countries": {"$in": countries }},{ "title": 1 ,'_id':1})
        return list(cursor)
...
def get_movies(filters, page, movies_per_page):
    ...
    if page == 0:
        ...
        movies = cursor.skip(int(page) * int(movies_per_page))\
...
def login_user(email, jwt):
    ...
    try:
        ...
        db.sessions.update_one( {"user-id": email }, {"$set": {"jwt": jwt}},
                                upsert=True)
        return {"success": True}
    except Exception as e:
        return {"error": e}
